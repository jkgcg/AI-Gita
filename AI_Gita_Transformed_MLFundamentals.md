...
> **What is an embedding?** An embedding is a way of converting any piece of data — a word, a sentence, a product, a customer — into a list of numbers that captures its *meaning* or *characteristics*. Items that are similar in meaning or behaviour end up with similar number lists; items that are different end up with very different number lists. You never read these numbers directly — the system uses them to calculate similarity (are these two things close to each other?) and to cluster items (which things belong together?). Embeddings are how search engines find semantically similar documents, how recommendation systems find similar products, and how RAG systems find relevant knowledge base entries. Think of it as a universal coordinate system for meaning.
